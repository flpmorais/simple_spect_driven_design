...
5a. If `{review_mode}` = `"full"`, validate mandatory story metadata before proceeding:
   - The first line of `{spec_file}` must be `branch:<branchidentifier>`.
   - The story must contain `## ARCHITECTURE MANDATES — NON-NEGOTIABLE` before `## Acceptance Criteria`.
   - Load `_bmad-output/planning-artifacts/architecture.md` and `_bmad-output/planning-artifacts/prd.md` as mandatory context docs.
   - If the branch line or mandates section is missing/malformed, HALT and notify the user immediately.
   - If the story mandates conflict with architecture or PRD, HALT and notify the user immediately.
