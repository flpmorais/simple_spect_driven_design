...
3. **Classify** each finding into exactly one bucket:
   - **architecture_stop** -- The reviewed code is not aligned with the story file, the story's architecture mandates, the architecture file, or the PRD. This is a non-negotiable STOP condition.
...
4. If any `architecture_stop` finding exists, present those findings first and HALT immediately. Notify the user that review cannot continue, patch, mark done, commit, or push until alignment is resolved.
