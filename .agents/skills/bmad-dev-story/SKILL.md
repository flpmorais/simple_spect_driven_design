...
<workflow>
  ...
  <critical>Architecture and PRD are binding. If architecture or PRD says X and you want to do Y, STOP and ask. Do not proceed silently.</critical>
  ...
  <step n="1" goal="Find next ready story and load it" tag="sprint-status">
    ...
    <action>Verify the first line of the story file is `branch:<branchidentifier>` and the section `## ARCHITECTURE MANDATES — NON-NEGOTIABLE` appears before `## Acceptance Criteria`.</action>
    <action if="branch line or architecture mandates section is missing or malformed">HALT: "Story file is missing required branch metadata or architecture mandates"</action>
    <action>Load `_bmad-output/planning-artifacts/architecture.md` and `_bmad-output/planning-artifacts/prd.md` completely enough to validate the story's architecture mandates and PRD constraints relevant to this story.</action>
    <action if="story mandates conflict with architecture or PRD">HALT: "Story mandates conflict with architecture/PRD; user decision required"</action>
  ...
  <step n="5" goal="Implement task following red-green-refactor cycle">
    ...
    <action>Before planning implementation, compare the intended approach with the story's architecture mandates, architecture file, and PRD.</action>
    <action if="the intended approach deviates from story mandates, architecture, or PRD">HALT: "Architecture/PRD deviation detected; ask user before proceeding"</action>
