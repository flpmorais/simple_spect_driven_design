...
- `branch_identifier` = current git branch name
...
<step n="2" goal="Load and analyze core artifacts">
  ...
  <critical>Architecture validation is mandatory. If the architecture file is missing, incomplete for this story, or conflicts with the PRD/epics, HALT and notify the user. Do not create the story.</critical>
  <action>Determine the current git branch name and set {{branch_identifier}} to it. If git branch detection fails, HALT and notify the user.</action>
...
<step n="3" goal="Architecture analysis for developer guardrails">
  ...
  <critical>The extracted architecture mandates are non-negotiable. Store them as {{architecture_mandates}} and write them into a top-level story section named exactly `## ARCHITECTURE MANDATES — NON-NEGOTIABLE` before `## Acceptance Criteria`.</critical>
  <action>Validate each mandate against PRD and epic requirements. If architecture says X and PRD/epic says incompatible Y, HALT and ask the user to resolve the conflict.</action>
  <action>Include explicit "do not" mandates for forbidden deviations, especially new architecture components, framework/library choices, containers, API patterns, persistence patterns, security behavior, and data flows not allowed by architecture/PRD.</action>
  <action>If no story-specific mandates are found, write a concise mandate stating the story must preserve existing architecture and list the specific architecture sections reviewed. Do not leave a generic placeholder.</action>
...
<step n="5" goal="Create comprehensive story file">
  ...
  <template-output file="{default_output_file}">architecture_mandates</template-output>
  ...
  <action>Verify the first line of {default_output_file} is exactly `branch:{{branch_identifier}}`.</action>
  <action>Verify `## ARCHITECTURE MANDATES — NON-NEGOTIABLE` appears before `## Acceptance Criteria` and contains the validated story-specific architecture mandates. If validation fails, fix the story before continuing.</action>
